...
# API Gateway ###########################     
    def send_df_to_frontend_in_chunks(self, df, connection_id, chunk_size):
        """
        ...
        self.api_client.post_to_connection(
            ...
            Data=json.dumps({"label": "headers", "data": headers}).encode("utf-8")
        )
        ...
        for row in csv_reader:
            ...
            if len(chunk) >= chunk_size:
                self.api_client.post_to_connection(
                    ConnectionId=connection_id,
                    Data=json.dumps({"label": "chunk", "data": chunk}).encode("utf-8")
                )
                chunk = []
        ...
        if chunk:
            self.api_client.post_to_connection(
                ConnectionId=connection_id,
                Data=json.dumps({"label": "last_chunk", "data": chunk}).encode("utf-8")
            )
